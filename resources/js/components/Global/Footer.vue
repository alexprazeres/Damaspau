<template>
   <div>
      <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@iconscout/unicons@3.0.6/css/line.css" />
      <footer class="bg-gray-800 sm:mt-10 pt-10">
         <div class="max-w-6xl m-auto text-gray-800 flex flex-wrap justify-left">
            <!-- Col-1 -->
            <div class="p-5 w-1/2 sm:w-4/12 md:w-4/12">
               <!-- Col Title -->
               <div class="text-xs uppercase text-gray-400 font-medium mb-6">
                  <img src="/images/logowhite.png" width="150" />
               </div>
               <p class="text-white">
                  O point Conceito da cidade 🕺🔥💃<br />
                  R. Henrique Dias, N.899 - Parque Pra Morar 2, Santa Inês - Ma.<br />
                  (Rua do Colégio CAIC, A Terceira Rua à Direita)<br />
                  Aberto de Terça a domingo 18hrs
               </p>
            </div>
            <!-- Col-3 -->
            <div class="p-5 w-1/2 sm:w-4/12 md:w-4/12">
               <!-- Col Title -->
               <div class="text-xs uppercase text-gray-400 font-medium mb-6">
                  LINKS RÁPIDOS
               </div>
               <!-- Links -->
               <Link href="/" class="
                  my-3
                  block
                  text-gray-300
                  hover:text-gray-100
                  text-sm
                  font-medium
                  duration-700
                  ">
               Início
               </Link>
                <Link href="/modelos"  class="
                  my-3
                  block
                  text-gray-300
                  hover:text-gray-100
                  text-sm
                  font-medium
                  duration-700
                  ">
               Modelos
               </Link>
                <Link href="/acasa" class="
                  my-3
                  block
                  text-gray-300
                  hover:text-gray-100
                  text-sm
                  font-medium
                  duration-700
                  ">
               A Casa
               </Link>
               <Link href="/contatos" class="
                  my-3
                  block
                  text-gray-300
                  hover:text-gray-100
                  text-sm
                  font-medium
                  duration-700
                  ">
               Contatos
               </Link>

            </div>
            <!-- Col-3 -->
            <div class="p-5 w-1/2 sm:w-4/12 md:w-4/12">
               <!-- Col Title -->
               <div class="text-xs uppercase text-gray-400 font-medium mb-6">
                  Últimos Eventos
               </div>
            </div>
         </div>
         <!-- Copyright Bar -->
         <div class="pt-2">
            <div class="
               flex
               pb-5
               px-3
               m-auto
               pt-5
               border-t border-gray-500
               text-gray-400 text-sm
               flex-col
               md:flex-row
               max-w-6xl
               ">
               <div class="mt-2">Damas de Paus 2021</div>
               <!-- Required Unicons (if you want) -->
               <div class="md:flex-auto md:flex-row-reverse mt-2 flex-row flex">
                  <a href="#" class="w-6 mx-1">
                  <i class="uil uil-facebook-f"></i>
                  </a>
                  <a href="#" class="w-6 mx-1">
                  <i class="uil uil-instagram"></i>
                  </a>
               </div>
            </div>
         </div>
      </footer>
   </div>
</template>
<script>
import {
    Link
} from '@inertiajs/inertia-vue'
   export default {
       components:{
           Link
       }
   };
</script>
<style></style>
